...
void SetUpConfig()
{
    std::filesystem::path filePath = std::filesystem::path(getenv(LIT("USERPROFILE"))) / LIT("Documents") / LIT("BattlebitDMA") / (LIT("Default.json"));
...
void SaveConfig(const std::wstring& config)
{
    ...
    std::filesystem::path filePath = std::filesystem::path(getenv(LIT("USERPROFILE"))) / LIT("Documents") / LIT("BattlebitDMA") / (cfg + LIT(".json"));
...
std::vector<std::wstring> GetAllConfigs()
{
    ...
    std::filesystem::path directory = std::filesystem::path(getenv(LIT("USERPROFILE"))) / LIT("Documents") / LIT("BattlebitDMA");
